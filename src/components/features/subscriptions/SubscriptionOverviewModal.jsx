      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="relative bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700/50 rounded-2xl shadow-2xl w-full max-w-lg p-8 transform transition-all">
          ...
          <div className="text-center mb-8">
            <h2 id="overview-title" className="text-xl font-bold text-white">
          ...
          {!stats ? (
            <p className="text-center text-gray-400">No subscriptions to analyze.</p>
          ) : (
            <div className="space-y-8">
              {/* Billing Cycle Split */}
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <svg className="w-4 h-4 text-emerald-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
                    <path d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                  </svg>
                  <span className="text-sm font-medium text-gray-300">Billing Cycle</span>
                </div>
                <p className="text-xs text-gray-500 mb-3">
                  How your subscriptions are billed — monthly vs yearly plans.
                </p>
                <div className="flex gap-1 h-3 rounded-full overflow-hidden bg-gray-700/50">
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <svg className="w-4 h-4 text-teal-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <span className="text-sm font-medium text-gray-300">Cost Comparison</span>
                </div>
                <p className="text-xs text-gray-500 mb-3">
                  All prices shown as monthly cost for easy comparison.
                </p>
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-gray-800/60 border border-gray-700/30 rounded-xl p-3 text-center">
                    <div className="w-7 h-7 rounded-lg bg-emerald-500/10 flex items-center justify-center mx-auto mb-2">
                      <svg className="w-3.5 h-3.5 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
                        <path d="M18 15l-6-6-6 6" />
                      </svg>
                    </div>
                    <p className="text-[11px] text-gray-500 mb-1">Highest</p>
                    <p className="text-sm font-bold text-white truncate" title={stats.mostExpensive.name}>
                  ...
                  <div className="bg-gray-800/60 border border-gray-700/30 rounded-xl p-3 text-center">
                    <div className="w-7 h-7 rounded-lg bg-teal-500/10 flex items-center justify-center mx-auto mb-2">
                      <svg className="w-3.5 h-3.5 text-teal-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
                        <path d="M6 9l6 6 6-6" />
                      </svg>
                    </div>
                    <p className="text-[11px] text-gray-500 mb-1">Lowest</p>
                    <p className="text-sm font-bold text-white truncate" title={stats.cheapest.name}>
                      {stats.cheapest.name}
                    </p>
              ...
              {stats.topCategory && (
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <svg className="w-4 h-4 text-lime-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <span className="text-sm font-medium text-gray-300">Most Used Category</span>
                  </div>
                  <div className="flex items-center justify-between bg-gray-800/60 border border-gray-700/30 rounded-xl px-4 py-3.5">
                    <span className="text-sm font-semibold text-white">{stats.topCategory[0]}</span>
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <svg className="w-4 h-4 text-yellow-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
                    <path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9" />
                    <path d="M13.73 21a2 2 0 01-3.46 0" />
                  </svg>
                  <span className="text-sm font-medium text-gray-300">Renewing Soon</span>
                </div>
                <p className="text-xs text-gray-500 mb-3">
                  Subscriptions renewing within the next 7 days.
                </p>
                {stats.upcoming.length === 0 ? (
                  <div className="bg-gray-800/60 border border-gray-700/30 rounded-xl px-4 py-5 text-center">
                    ...
                    <p className="text-sm text-gray-400">No upcoming renewals this week</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {stats.upcoming.map((sub) => (
                      <div
                        key={sub.id}
                        className="flex items-center justify-between bg-gray-800/60 border border-gray-700/30 rounded-xl px-4 py-2.5"
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          {sub.icon ? (
                            ...
                            <div className="w-7 h-7 rounded-lg bg-gray-700 flex items-center justify-center text-[10px] font-bold text-gray-400 shrink-0">
                              {sub.name[0]}
                            </div>
                          )}
                          <div className="min-w-0">
                            <p className="text-sm text-white font-medium truncate">{sub.name}</p>
                            <p className="text-[11px] text-gray-500">{formatDate(sub.renewal)}</p>
                          </div>
                        </div>
